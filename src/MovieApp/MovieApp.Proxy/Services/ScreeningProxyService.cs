...
namespace MovieApp.Proxy.Services
{
    public class ScreeningProxyService : IScreeningService
    {
        ...
        {
            return await _apiClient.GetAsync<Screening>($"api/screenings/{screeningId}");
        }
        ...
        {
            var result = await _apiClient.GetAsync<List<Screening>>($"api/screenings?eventId={eventId}");
        ...
        {
            var result = await _apiClient.GetAsync<List<Screening>>($"api/screenings?movieId={movieId}");
            return result ?? new List<Screening>();
        }

        public async Task<IReadOnlyList<Seat>> GetAvailableSeatsAsync(int screeningId, CancellationToken cancellationToken = default)
        {
            var result = await _apiClient.GetAsync<List<Seat>>($"api/screenings/{screeningId}/seats");
        ...
        {
            await _apiClient.PostAsync("api/screenings", screening);
        }
    }
}
