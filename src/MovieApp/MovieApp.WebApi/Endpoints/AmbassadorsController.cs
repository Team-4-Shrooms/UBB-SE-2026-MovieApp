...
public sealed class AmbassadorsController : ControllerBase
{
    ...
    [HttpGet("history/{ambassadorId:int}")]
    public async Task<IActionResult> GetReferralHistory(int ambassadorId)
    {
        var history = await _ambassadorService.GetReferralHistoryAsync(ambassadorId);
        return Ok(history);
    }
